	...
	public function __construct() 
	{
		...
		$this->oauth_secret = $CI->input->cookie($config . "oauthsecret", TRUE);
 		...
 		$select = "users.`ID`, users.`username`, users.`email`, 
				...
				users.noti_count, users.email_count,
				users.user_role, user_roles.name as ur_name,
				users.address_line_1, users.mobile_phone,
				users.city, users.state, users.phone, users.country,
				user_roles.admin, user_roles.admin_settings, 
				user_roles.admin_members, user_roles.admin_payment,
				user_roles.banned, user_roles.teacher, user_roles.student,
				user_roles.subject_manager, user_roles.subject_viewer,
				user_roles.student_group_manager, 
				user_roles.student_group_viewer, user_roles.class_manager,
				user_roles.class_viewer, user_roles.library_manager,
				user_roles.library_viewer, user_roles.student_manager,
				user_roles.student_viewer, user_roles.invoice_manager,
				user_roles.invoice_viewer, user_roles.behaviour_manager,
				user_roles.behaviour_viewer, user_roles.announcement_manager,
