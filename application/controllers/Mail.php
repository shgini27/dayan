...
class Mail extends CI_Controller 
{
	...
	public function view_mail($id, $page=0) 
	{
		...
		if($mail->num_rows() == 0) {
			$this->template->errori(lang("error_150"));
		}
		$mail = $mail->row();
	...
	public function compose_pro() 
	{
		...
		if(empty($title)) $this->template->error(lang("error_85"));
		if(empty($reply)) $this->template->error(lang("error_151"));

		// Check user is good
		$user = $this->user_model->get_user_by_username($username);
		if($user->num_rows() == 0) $this->template->error(lang("error_152"));
		$user = $user->row();
	...
	public function remove_block($id, $hash) 
	{
		...
		if($block->num_rows() == 0) $this->template->error(lang("error_153"));
