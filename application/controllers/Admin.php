...
class Admin extends CI_Controller 
{
	...
	public function school_reset() 
	{
		...
		$debug = "";
		if(isset($_POST['s'])) {
			if($_POST['remove_assignments']) {
				$this->db->truncate("class_assignments");
				$debug .= "Class Assignments cleared! <br />";
			}
			if($_POST['remove_attendance']) {
				$this->db->truncate("attendance_sheet_entries");
				$debug .= "Attendance cleared! <br />";
			}
			if($_POST['remove_students']) {
				$this->db->truncate("class_students");
				$debug .= "Class Students cleared! <br />";
			}
			if($_POST['remove_timetable']) {
				$this->db->truncate("calendar_events");
				$debug .= "Timetable cleared! <br />";
			}
			if($_POST['remove_class_announcements']) {
				$this->db->truncate("class_announcements");
			...
			if($_POST['remove_class']) {
				...
				$this->db->truncate("class_files");
				$this->db->truncate("class_grades");
				$debug .= "Class cleared! <br />";
			}
			if($_POST['remove_subjects']) {
				$this->db->truncate("subjects");
				$this->db->truncate("subject_teachers");
				$debug .= "Subjects cleared! <br />";
			}
		...
		$this->template->loadData("activeLink", 
			array("admin" => array("tools" => 1)));
		$this->template->loadContent("admin/tool_school_reset.php", array(
			"debug" => $debug
			)
		);
	}
	...
	public function section_settings() 
	{
		...
		$this->template->loadData("activeLink", 
			array("admin" => array("section_settings" => 1)));
	...
	public function section_settings_pro() 
	{
		...
		$announcements_section = intval($this->input->post("announcements_section"));
		$classes_section = intval($this->input->post("classes_section"));
		$subjects_section = intval($this->input->post("subjects_section"));
		$students_section = intval($this->input->post("students_section"));
		$files_section = intval($this->input->post("files_section"));
		$finance_section = intval($this->input->post("finance_section"));
		$invoices_section = intval($this->input->post("invoices_section"));
		$behaviour_section = intval($this->input->post("behaviour_section"));
		$library_section = intval($this->input->post("library_section"));
		$reports_section = intval($this->input->post("reports_section"));
		$parent_section = intval($this->input->post("parent_section"));
		$hostel_section = intval($this->input->post("hostel_section"));
                //custom added
                $documents_section = intval($this->input->post("documents_section"));
	...
	public function invoice_settings() 
	{
		...
		$this->template->loadData("activeLink", 
			array("admin" => array("invoice_settings" => 1)));

		$invoice = $this->invoices_model->get_invoice_settings();
		$invoice = $invoice->row();

		$this->template->loadContent("admin/invoice_settings.php", array(
			"invoice" => $invoice
			)
		);
	}

	public function invoice_settings_pro() 
	{
		if(!$this->common->has_permissions(array("admin", "admin_settings"),
		 $this->user)) {
			$this->template->error(lang("error_2"));
		...
		redirect(site_url("admin/invoice_settings"));
	}
	...
	{	
		...
		$this->template->loadData("activeLink", 
			array("admin" => array("school_settings" => 1)));
		$this->template->loadContent("admin/school_settings.php", array(
			)
		);

	}

	public function school_settings_pro() 
	{
		if(!$this->common->has_permissions(array("admin", "admin_settings"),
		 $this->user)) {
			$this->template->error(lang("error_2"));
		}
		$student_view_groups = intval($this->input->post("student_view_groups"));
		$teacher_group_manage = intval($this->input->post("teacher_group_manage"));
		$teacher_class_manage = intval($this->input->post("teacher_class_manage"));
		$teacher_class = intval($this->input->post("teacher_class"));
		$reserve_books = intval($this->input->post("reserve_books"));
		$allow_student_profile = intval($this->input->post("allow_student_profile"));

		// update
		$this->admin_model->updateSettings(
			array(
				"student_view_groups" => $student_view_groups,
				"teacher_group_manage" => $teacher_group_manage,
				"teacher_class_manage" => $teacher_class_manage,
				"teacher_class" => $teacher_class,
				"reserve_books" => $reserve_books,
				"allow_student_profile" => $allow_student_profile
			)
		);
		$this->session->set_flashdata("globalmsg", lang("success_13"));
		redirect(site_url("admin/school_settings"));
	}
	...
	public function add_custom_field_pro() 
	{
		...
		$school_report = intval($this->input->post("school_report"));
	...
	{
		if($hash != $this->security->get_csrf_hash()) {
			$this->template->error(lang("success_6"));
		}
		$id = intval($id);
		$id = intval($id);
	...
	public function settings_pro() 
	{
		...
		$fp_currency_symbol = $this->common->nohtml($this->input->post("fp_currency_symbol"));
